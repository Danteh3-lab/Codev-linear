...
`;
    ...
    class LaserFlowBridge {
        constructor() {
            this.shell = document.querySelector('.laser-flow-bridge-shell');
            ...
            this.videoContainer = document.getElementById('laser-flow-bridge-video');
            this.mobileContainer = document.getElementById('laser-flow-bridge-mobile');
            this.video = this.videoContainer ? this.videoContainer.querySelector('video') : null;
        ...
        init() {
            if (!this.shell || !this.target || this.shell.dataset.laserReady === 'true') {
                return;
            }

            this.shell.dataset.laserReady = 'true';
            this.bindEvents();
            this.handleResize();
            this.resetReveal();
        }
        ...
        buildScene() {
            if (this.renderer || !this.mobileContainer || !this.THREE) {
                return;
            }
            ...
            this.mobileContainer.appendChild(renderer.domElement);
            ...
            this.uniforms = {
                ...
                uBeamYFrac: { value: -0.24 },
                uFlowSpeed: { value: 0.35 },
                uVLenFactor: { value: 4.2 },
                uHLenFactor: { value: 1.56 },
                uFogIntensity: { value: 0.94 },
                uFogScale: { value: 0.3 },
                uWSpeed: { value: 15.0 },
                uWIntensity: { value: 10.2 },
        ...
        handlePreferenceChange() {
            ...
            const enabled = this.pointerFineQuery.matches && !this.reduceMotionQuery.matches && !this.isMobile();
        ...
        handlePointerMove(event) {
            ...
            if (clientX < zoneRect.left || clientX > zoneRect.right || clientY < zoneRect.top || clientY > zoneRect.bottom) {
                this.resetReveal();
                return;
            }

            this.revealLayer.dataset.active = 'true';
            this.revealLayer.style.setProperty('--mx', `${clientX - zoneRect.left}px`);
            this.revealLayer.style.setProperty('--my', `${clientY - zoneRect.top}px`);
        }
        ...
        handleResize() {
            ...
            this.resizeFrame = window.requestAnimationFrame(() => {
                ...
                this.buildScene();
                if (!this.renderer || !this.uniforms || !this.mobileContainer) {
                    return;
                }

                const width = Math.max(this.mobileContainer.clientWidth, 1);
                const height = Math.max(this.mobileContainer.clientHeight, 1);
        ...
        handleVisibility() {
            ...
            if (this.active && this.isMobile() && this.renderer && !this.frameId) {
                this.frameId = window.requestAnimationFrame(this.animate);
            }
        ...
        animate() {
            if (!this.renderer || !this.uniforms || !this.clock) {
                this.frameId = null;
                return;
            }
    ...
    window.initLaserFlowBridge = function initLaserFlowBridge() {
        ...
        const bridge = new LaserFlowBridge();
        bridge.init();
